...
figure(1);
imshow(uint8(image));
% image_normalized=round(image./255); %normalizing betweem 0 and 1

N=3;   %filter size

img_copied = image_normalized;
thinned_out=zeros(row,col);
...
while f
    ...
    for i=1:row
        for j=1:col
            thinned_out(i,j)=img_copied(i,j) & (~Mark_array(i,j) | Uncond_mark_array(i,j));
        end
    end
     if(img_copied==thinned_out)
        f=0;
    else
        f=1;
    end
    
    img_copied=thinned_out;
    
end

figure(2);
imshow(uint8(thinned_out.*255));
